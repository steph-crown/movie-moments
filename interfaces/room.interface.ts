...
/* eslint-disable @typescript-eslint/no-explicit-any */
// interfaces/room.interface.ts - Updated with homepage support
...
export interface RoomParticipant {
  ...
  email: string | null;
  username: string | null;
  status: "pending" | "joined" | "left"; // Added "left" status
  role: "creator" | "member";
  join_method: "created" | "invited_email" | "invited_username" | "public_link";
  joined_at: string | null;
  last_seen: string | null;
...
// Updated ICachedContent interface with homepage
export interface ICachedContent {
  id: string;
  tmdb_id: number;
  content_type: RoomContentType;
  ...
  number_of_seasons?: number; // Only for series
  number_of_episodes?: number; // Only for series
  seasons?: any[]; // Include seasons for position selector
  genres: any[];
  first_air_date?: string;
  homepage?: string | null; // NEW: Homepage URL from TMDB
  platforms?: string[];
  cached_at: string;
  last_accessed: string;
}
